...
**最後更新**：2025-12-18（調整簽到獎勵：首次 100 MGC，後續 20 MGC）
...
1. **Given** 使用者錢包已連接且今天（UTC+8）尚未簽到且非首次簽到，**When** 點擊「在訪客簿上簽名」按鈕，**Then** 發起鏈上交易，成功後獲得 20 MGC，餘額數字跳動更新並顯示 +20 MGC 動畫
2. **Given** 新使用者第一次簽到，**When** 簽到成功，**Then** 系統建立 UserCheckInRecord 物件，發放 100 MGC（新用戶禮包），顯示特別歡迎訊息「歡迎來到永恆圖書館！獲得 100 智慧碎片」
...
- **FR-007**：系統必須在首次簽到時發放 100 個 MGC（新用戶禮包），後續每日簽到發放 20 個 MGC（鏈上 Coin）
...
- 首次簽到獎勵 100 MGC，後續每日簽到獎勵 20 MGC，每次抽取消耗固定為 10 MGC
...
| Story 2 - 每日簽到 | ✅ 包含 | UTC+8 日期制，首次 100 MGC / 後續 20 MGC |
